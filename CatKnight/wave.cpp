...
#define WAVE_HEIGHT			(25.0f)	//高さ
#define WAVE_RADIUS			(30.0f)	//半径
...
void UpdateWave(void)
{
	...
	case WAVE_STATE_ATTACK:
		g_wave.fRadius += float(WAVE_RADIUS_MAX / WAVE_SPREAD_TIME);
		if (g_wave.fRadius > WAVE_RADIUS_MAX)
		{	//広がりきったら元に戻る
			g_wave.state = WAVE_STATE_CHARGE;
			pPrincess->stateOld = pPrincess->state;
			...
			g_wave.fRadius = WAVE_RADIUS;
			g_wave.col = D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f);
			pMagic->col = D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f);
		}
		break;
	default:
		break;
